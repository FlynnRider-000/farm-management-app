...
export const farmReducer = (state = INITIAL_STATE, action: any) => {
  switch (action.type) {
    case FARM_TYPES.CREATE_ASSESSMENT:
      const ffs = state.allFarms!.map((farm) => {
    ...
    case FARM_TYPES.UPDATE_ASSESSMENT:
      const ffs1 = state.allFarms!.map((farm) => {
        if (farm.id === action.payload.farm_id) {
          let newFarm = {...farm};
          newFarm.lines = [];
          for (let i = 0; i < farm.lines.length; i++) {
    ...
    case FARM_TYPES.CREATE_SEEDING:
      const ffs2 = state.allFarms!.map((farm) => {
        if (farm.id === action.payload.farm_id) {
          let newFarm = {...farm};
          newFarm.lines = [];
          for (let i = 0; i < farm.lines.length; i++) {
    ...
    case FARM_TYPES.CREATE_HARVEST:
      const ffs3 = state.allFarms!.map((farm) => {
        if (farm.id === action.payload.farm_id) {
          let newFarm = {...farm};
          newFarm.lines = [];
          for (let i = 0; i < farm.lines.length; i++) {
